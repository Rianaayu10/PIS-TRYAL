...
namespace API_PIS4.Controllers
{
    ...
    public class UserSetupController : ControllerBase
    {
        ...
        UserSetupDB db = new UserSetupDB();
        public UserSetupController(IConfiguration config)
        {
            _config = config;
        }

        [HttpPost]
        public ActionResult GetList([FromBody] UserSetup prm)
        {
        ...
        [HttpPost]
        public ActionResult Update([FromBody] UserSetup prm)
        {
            constr = _config.GetConnectionString("DefaultConnection");
            Response resp = new Response();
            try
